...
import { emitPrismSignal } from "@/lib/cbse-class-10-english/cortex-signals";

describe("PRISM Signals", () => {
  it("dispatches custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ eventType: "question_answered", activityType: "cbse-class-10-english" });
  ...
  it("includes correct event detail", () => {
    ...
    emitPrismSignal({ eventType: "exam_started", value: 1, activityType: "cbse-class-10-english" });
    expect(detail).toEqual({ eventType: "exam_started", value: 1, activityType: "cbse-class-10-english" });
