...
import { EXAM_CONFIG } from "@/lib/cbse-class-10-english/config";

describe("CBSE Class 10 English Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM187");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("cbse-class-10-english");
  });
